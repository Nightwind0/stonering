...
void MappableObjectElement::load_attributes( CL_DomNamedNodeMap attributes ) {
	...
	if ( size == "small" )
		m_size = DimensionsFromSizeType( MO_SMALL );
	else if ( size == "medium" )
		m_size = DimensionsFromSizeType( MO_MEDIUM );
	else if ( size == "large" )
		m_size = DimensionsFromSizeType( MO_LARGE );
	else if ( size == "wide" )
		m_size = DimensionsFromSizeType( MO_WIDE );
	else if ( size == "tall" )
		m_size = DimensionsFromSizeType( MO_TALL );
	else {
	...
	if ( motype == "npc" )
		m_eType = NPC;
	else if ( motype == "square" )
		m_eType = SQUARE;
	else if ( motype == "container" )
		m_eType = CONTAINER;
	else if ( motype == "door" )
		m_eType = DOOR;
	else if ( motype == "warp" )
...
CL_DomElement MappableObjectElement::CreateDomElement( CL_DomDocument& doc ) const {
	...
	if ( IsSolid() )
		element.set_attribute( "solid", "true" );
