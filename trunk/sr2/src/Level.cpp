...
MappableObject::MappableObject():meDirection(NONE),mpSprite(NULL),mpMovement(0),
                                 mpCondition(0),cFlags(0),mnCellsMoved(0),mnMovesSkipped(0)
{
...
void Level::moveMappableObjects()
{
...
// So this should be called periodically, but not every MO will move when it's called.
        ...
        for(std::list<MappableObject*>::iterator i = mMappableObjects.begin();
            ...
            i++)
        {
                if(! (*i)->evaluateCondition()) continue; // Skip 'em

                if((*i)->chanceToMove())
                {

                        if((*i)->getDirection() != MappableObject::NONE)
                        {
                                ...
                                if((*i)->isAligned())
                                {
                                        if((*i)->isSolid())
                                        {
                                                ...
                                                CL_Point topleft = (*i)->getPositionAfterMove();
                                        
                                                MappableObject::CalculateEdgePoints(topleft, (*i)->getDirection(),
                                                                                    (*i)->getSize(), &intoPoints);
                                                ...
                                                for(std::list<CL_Point>::iterator iter = intoPoints.begin();
                                                    ...
                                                    iter++)
                                                {
                                                        if(mOccupiedTiles.count(*iter)
                                                           ||
														   (getCumulativeDirectionBlockAtPoint(*iter) & MappableObject::ConvertDirectionToDirectionBlock((*i)->getDirection()))
                                                           ||
                                                ...
                                                for(std::list<CL_Point>::iterator iter = intoPoints.begin();
                                                    ...
                                                    iter++)
                                                {
                                                        mOccupiedTiles[*iter] = true;
                                                }
                                        }
                                
                                        // Okay. We've made sure that we can move. We've marked where we're going as occupied.
                                        (*i)->move();
                                        ...
                                        if((*i)->isAligned())
                                        {
                                                ...
                                                CL_Point topleft = (*i)->getPosition();
                                        
                                                MappableObject::CalculateEdgePoints(topleft, MappableObject::OppositeDirection((*i)->getDirection()),
                                                                                    (*i)->getSize(), &fromPoints);
...
bool Level::tryMove(const CL_Point &currently, const CL_Point & destination)
{
        ...
        if(destination.x <0 || destination.x > mLevelWidth) return false;
        if(destination.y <0 || destination.y > mLevelHeight) return false;

        eDirectionBlock directionBlock;

        if(currently.x < destination.x) // We're moving east, so we care about west blocks
                directionBlock = DIR_WEST;
        else if (currently.x > destination.x)
                directionBlock = DIR_EAST;
        else if (currently.y < destination.y) // NORTH
                directionBlock = DIR_NORTH;
        else directionBlock = DIR_SOUTH;

        if(mOccupiedTiles.count(destination)
           ||
           (getCumulativeDirectionBlockAtPoint(destination) & directionBlock)
                )
        {
                return false;
        }
        ...
        {
                mOccupiedTiles[currently] = false;
                mOccupiedTiles[destination] = true;

                step( destination , 1, 1);
...
void Level::step(const CL_Point &target, uint width, uint height)
{
        ...
        CL_Rect rect(target.x, target.y, target.x + width, target.x + height);
            ...
            i++)
        {
                ...
                if((*i)->evaluateCondition())
                {
                        CL_Point moPoint = (*i)->getPosition();
                        CL_Rect moRect = CL_Rect(moPoint.x, moPoint.y,
                                                 moPoint.x + (*i)->getCellHeight(),
                                                 moPoint.y + (*i)->getCellWidth());
	    
                        if( moRect.is_overlapped(rect))
                        {
...
void Level::talk(const CL_Point &target, bool prod)
{
        ...
        CL_Rect rect(target.x, target.y, target.x + 1, target.x + 1);
