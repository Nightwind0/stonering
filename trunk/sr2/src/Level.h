...
namespace StoneRing {
  ...
  class Level
    ...
    public:
      ...
      void draw(uint levelX, uint levelY, CL_GraphicContext * pGC,
		uint windowX = 0, uint windowY=0,
		uint windowWidth = Application::WINDOW_WIDTH,
		uint windowHeight = Application::WINDOW_HEIGHT);
  ...
  private:
      ...
      bool moSortCriterion( const MappableObject *p1, const MappableObject * p2);

      // Sort tiles on zOrder
      bool tileSortCriterion ( const Tile * p1, const Tile * p2);
