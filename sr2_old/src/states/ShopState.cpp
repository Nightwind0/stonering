...
clan::Rectf ShopState::ItemMenu::get_rect()
{
    return m_rect;
}
...
int ShopState::ItemMenu::height_for_option ( clan::Canvas& gc )
{
...
void ShopState::ItemMenu::SetRect ( const clan::Rectf& rect )
{
    m_rect = rect;
}
...
{    
    ...
    clan::Pointf item_point = GraphicsManager::GetPoint(GraphicsManager::SHOP,"item_box");
    
    clan::Rectf items_rect = m_items_rect;
...
void ShopState::Draw ( const clan::Rect& screenRect, clan::Canvas& GC )
{
...
void ShopState::draw_status ( clan::Canvas& gc )
{
    m_item_menu.Choose();
...
void ShopState::draw_characters ( clan::Canvas& gc )
{
    clan::Rectf rect = m_character_rect;
    ...
    {
        clan::Sprite pSprite = dynamic_cast<Character*>(party->GetCharacter(i))
...
void ShopState::draw_description ( clan::Canvas& gc )
{
...
void ShopState::draw_gold ( clan::Canvas& gc )
{
    clan::Rectf rect = m_gold_rect;
    rect.shrink ( GraphicsManager::GetMenuInset().x, GraphicsManager::GetMenuInset().y );
    std::ostringstream stream;
						...
						if(pChar->CanEquip(pEquipment)){
							...
							if(1 == IApplication::GetInstance()->DynamicMenu(options)){
								...
								for(int i=0;i<7;i++){
									...
									Equipment::eSlot slot = static_cast<Equipment::eSlot>(static_cast<int>(std::pow((float)2.0f,(float)i)));
