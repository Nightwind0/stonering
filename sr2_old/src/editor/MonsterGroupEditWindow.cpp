...
void MonsterGroupEditWindow::BattleWindow::on_render( clan::Canvas& gc, const clan::Rect& clip_rect )
{
	...
	for(int x=0;x<cols; x++){
		for(int y=0;y<rows; y++){
			clan::Colorf color;
			if( (y%2) == (x%2) )
				color = clan::Colorf(0.2f,0.2f,0.2f);
			else
				color = clan::Colorf(0.6f,0.6f,0.6f);
			gc.fill_rect(clan::Rect(clan::Point(x*cell_width,y*cell_height),clan::Size(cell_width,cell_height)),color);
		}
	}
	int id = 0;
	...
	for ( std::vector<MonsterRef*>::const_iterator it = monsters.begin();
		...
		if( m_selected_item == id++ ){
			clan::Rect rect(clan::Point(pRef->GetCellX()*cell_width,pRef->GetCellY()*cell_height),
						 clan::Size(cell_width*pRef->GetColumns(),cell_height*pRef->GetRows()));
			gc.draw_box(rect,clan::Colorf(1.0f,1.0f,1.0f));
		}

		for ( int y = 0;y < pRef->GetRows();y++ ) {
			for ( int x = 0;x < pRef->GetColumns();x++ ) {
				if ( count > 0 ) {
					clan::Sprite sprite = GraphicsManager::CreateMonsterSprite(pRef->GetName(),"idle");
...
MonsterGroupEditWindow::MonsterGroupEditWindow(clan::GUIManager* manager, const clan::GUITopLevelDescription& desc):clan::Window(manager,desc){
	m_battle_window = new BattleWindow(this);
	m_battle_window->set_geometry(clan::Rect(clan::Point(24,24),clan::Size(256,256)));
	m_battle_window->SetScale(1.0f/2.0f);
	m_monster_ref_list = new clan::ListView(this);
	m_monster_ref_list->set_geometry(clan::Rect(clan::Point(256+24+60,24),clan::Size(300,256)));
	...
	m_monsters = new clan::ComboBox(this);
	m_monsters->set_geometry(clan::Rect(clan::Point(256+24,256+24+12),clan::Size(160,24)));
	m_monsters->func_item_selected().set(this,&MonsterGroupEditWindow::on_monsters_selected);
	
	m_group_col_count = new clan::Spin(this);
	m_group_col_count->set_geometry(clan::Rect(clan::Point(12+128,256+24+12),clan::Size(60,18)));
	...
	m_group_row_count = new clan::Spin(this);
	m_group_row_count->set_geometry(clan::Rect(clan::Point(256+24,128+12),clan::Size(60,18)));
	...
	m_monster_count_label = new clan::Label(this);
	m_monster_count_label->set_geometry(clan::Rect(clan::Point(256+24+160+12,256+24+12+32),clan::Size(64,18)));
	m_monster_count_label->set_text("Count");
	m_monster_count = new clan::Spin(this);
	m_monster_count->set_geometry(clan::Rect(clan::Point(256+24+160+12,256+24+12),clan::Size(64,24)));
	...
	m_col_count_label = new clan::Label(this);
	m_col_count_label->set_geometry(clan::Rect(clan::Point(256+24+160+12+64+12,256+24+12+32),clan::Size(60,18)));
	m_col_count_label->set_text("Columns");
	m_col_count = new clan::Spin(this);
	m_col_count->set_geometry(clan::Rect(clan::Point(256+24+160+12+64+12,256+24+12),clan::Size(60,24)));
	m_col_count->set_ranges(1,10);
	m_col_count->set_value(1);
	m_row_count_label = new clan::Label(this);
	m_row_count_label->set_geometry(clan::Rect(clan::Point(256+24+160+12+64+12+32+12+22,256+24+12+32),clan::Size(60,18)));
	m_row_count_label->set_text("Rows");
	m_row_count = new clan::Spin(this);
	m_row_count->set_geometry(clan::Rect(clan::Point(256+24+160+12+64+12+32+12+22,256+24+12),clan::Size(60,24)));
	m_row_count->set_ranges(1,10);
	m_row_count->set_value(1);
	m_add_monsters = new clan::PushButton(this);
	m_add_monsters->set_geometry(clan::Rect(clan::Point(800-12-120,256+24+12),clan::Size(120,24)));
	...
	m_delete_monsters = new clan::PushButton(this);
	m_delete_monsters->set_geometry(clan::Rect(clan::Point(256+24+60+300,24+24+24+60),clan::Size(64,32)));
	...
	m_move_up->set_geometry(clan::Rect(clan::Point(256+24+60+300+32,24),clan::Size(32,24)));
	...
	m_move_down->set_geometry(clan::Rect(clan::Point(256+24+60+300+32,24+24+24),clan::Size(32,24)));
	...
	m_move_left = new clan::PushButton(this);
	m_move_left->set_geometry(clan::Rect(clan::Point(256+24+60+300,24+24),clan::Size(32,24)));
	...
	m_move_right = new clan::PushButton(this);
	m_move_right->set_geometry(clan::Rect(clan::Point(256+24+60+300+64,24+24),clan::Size(32,24)));
	...
	m_done_button = new clan::PushButton(this);
	m_done_button->set_geometry(clan::Rect(clan::Point(24,256+24+12+64),clan::Size(128,32)));
	m_done_button->set_text("Done");
	m_done_button->func_clicked().set(this,&MonsterGroupEditWindow::on_done);	
	m_group = NULL;
	
	clan::ListViewHeader *lv_header = m_monster_ref_list->get_header();
...
void MonsterGroupEditWindow::populate_monster_list()
{
	...
	for(std::list<MonsterElement*>::const_iterator it = monsters.begin();
		it != monsters.end(); it++){
	    clan::PopupMenuItem item = menu.insert_item((*it)->GetName());
	}
...
{
	clan::ListViewItem doc = m_monster_ref_list->get_document_item();
	doc.remove_children();
	int id = 0;
	for(std::vector<MonsterRef*>::const_iterator it = m_group->GetMonsters().begin();
		it != m_group->GetMonsters().end(); it++){
		clan::ListViewItem item = m_monster_ref_list->create_item();
...
void MonsterGroupEditWindow::on_monster_list_selection(clan::ListViewSelection selection)
{
	clan::ListViewItem item = m_monster_ref_list->get_selected_item();
	if(!item.is_null()){
	...
	if(m_monsters->get_selected_item() >= 0){
		...
		for(int x=m_group->GetCellColumns()-1;x>=0 && !found_spot;x--){
			for(int y=m_group->GetCellRows()-1;y>=0 && !found_spot;y--){
				clan::Rect candidate(clan::Point(x,y),clan::Size(pRef->GetColumns(),pRef->GetRows()));
		...
		if(found_spot){
			...
			for(std::vector<MonsterRef*>::const_iterator it = m_group->GetMonsters().begin();
						it != m_group->GetMonsters().end();it++){
				clan::Rect rect(clan::Point((*it)->GetCellX(),(*it)->GetCellY()),clan::Size((*it)->GetColumns(),(*it)->GetRows()));
				max_x = std::max(max_x,rect.right);
				max_y = std::max(max_y,rect.bottom);
			}
...
bool MonsterGroupEditWindow::can_place( const clan::Rect& candidate, MonsterRef* ignore )
{
	bool blocked = false;
...
void MonsterGroupEditWindow::sync_move_buttons()
{
	...
	const clan::Rect group_rect(clan::Point(0,0),clan::Size(m_group->GetCellColumns(),m_group->GetCellRows()));
	//m_delete_monsters->set_enabled(false);
	clan::ListViewItem item = m_monster_ref_list->get_selected_item();
	if(!item.is_null()){
		MonsterRef* ref = (MonsterRef*)m_group->GetMonsters()[item.get_id()];
		clan::Rect rect(clan::Point(ref->GetCellX(),ref->GetCellY()),clan::Size(ref->GetColumns(),ref->GetRows()));
		clan::Rect right,left,up,down;
