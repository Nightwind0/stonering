...
SR_FileDialog_Generic::SR_FileDialog_Generic(
	SR_FileDialog *self,
	...
	: filedialog(self)
{
	...
	treeview_files = new SR_TreeView(CL_Rect(10, 32, width - 10, 197), client_area);
	...
	slots.connect(self->sig_set_options(), this, &SR_FileDialog_Generic::on_set_options);
	slots.connect(treeview_files->sig_selection_changed(), this, &SR_FileDialog_Generic::on_file_activated);
	slots.connect(button_ok->sig_clicked(), this, &SR_FileDialog_Generic::on_button_quit, true);
	slots.connect(button_cancel->sig_clicked(), this, &SR_FileDialog_Generic::on_button_quit, false);
	slots.connect(button_parent->sig_clicked(), this, &SR_FileDialog_Generic::on_button_parent);
	slots.connect(button_createdir->sig_clicked(), this, &SR_FileDialog_Generic::on_button_createdir);
	slots.connect(input_file->sig_changed(), this, &SR_FileDialog_Generic::on_edit_file);
	slots.connect(input_filter->sig_return_pressed(), this, &SR_FileDialog_Generic::on_edit_filter);
	slots.connect(input_dir->sig_return_pressed(), this, &SR_FileDialog_Generic::on_edit_dir);
...
SRFileInfo::SRFileInfo(std::string name, bool directory, bool readable, bool writable, bool hidden, int size) 
...
const std::string &SR_FileDialog_Generic::get_file() const
{
	return file;
}

const std::string &SR_FileDialog_Generic::get_filter() const
{
	return filter;
}

const std::string &SR_FileDialog_Generic::get_dir() const
{
	return dir;
}

const std::string &SR_FileDialog_Generic::get_path() const
{
	throw CL_Error("SR_FileDialog_Generic::get_path() not implemented");
}

bool SR_FileDialog_Generic::is_hidden_files_visible() const
{
	return show_hidden;
}
...
void SR_FileDialog_Generic::set_file(const std::string &new_file, bool perform_refresh)
{
...
void SR_FileDialog_Generic::set_dir(const std::string &new_dir, bool perform_refresh)
{
...
void SR_FileDialog_Generic::set_filter(const std::string &new_filter, bool perform_refresh)
{
...
void SR_FileDialog_Generic::show_hidden_files(bool enable)
{
	show_hidden = enable;
}

void SR_FileDialog_Generic::refresh()
{
	...
	while (scanner.next())
	{
		...
		else 
		{
			...
			{	
				...
				{
					Directories.push_back(SRFileInfo(file, scanner.is_directory(), scanner.is_readable(), scanner.is_writable(), scanner.is_hidden(), 0));
				}
			}
		}
	}
	...
	while (scanner.next())
	{
		...
		else 
		{
			...
			{	
				if (!scanner.is_directory())
				{
					Files.push_back(SRFileInfo(file, scanner.is_directory(), scanner.is_readable(), scanner.is_writable(), scanner.is_hidden(), scanner.get_size()));
				}
			}
		}
	}

	std::sort(Directories.begin(), Directories.end(), SR_FileDialog_Generic::sort);
	std::sort(Files.begin(), Files.end(), SR_FileDialog_Generic::sort);
...
void SR_FileDialog_Generic::on_file_activated(const SR_TreeNode &node)
{
	SR_TreeItem *treeitem = (SR_TreeItem *)node.get_component();
...
void SR_FileDialog_Generic::on_button_quit(bool ok)
{
	if(ok == false)
		file = "";
...
void SR_FileDialog_Generic::on_button_parent()
{
	CL_Directory::change_to("..");
	read_dir();
}

void SR_FileDialog_Generic::on_button_createdir()
{
...
void SR_FileDialog_Generic::on_edit_dir()
{
	set_dir(input_dir->get_text());
}

void SR_FileDialog_Generic::on_edit_file(const std::string &str)
{

	set_file(input_file->get_text());
}

void SR_FileDialog_Generic::on_edit_filter()
{
	set_filter(input_filter->get_text());
}

void SR_FileDialog_Generic::on_set_options(const CL_DomElement &options)
{
...
void SR_FileDialog_Generic::read_dir()
{
...
bool SR_FileDialog_Generic::sort(const SRFileInfo &a, const SRFileInfo &b)
{
